...
{block name="frontend_index_content"}
    {if $receivedAddress == 'YES'}
        ...
        <br />- {s name="SendExactly"}Send exactly{/s} <strong>{$valueInBCH} BCH</strong> {s name="ToThisBitcoinCashAddress"}to this BitcoinSV Address{/s}:
        <div style="padding: 5px"><a target="_blank" style="background-color: white;" href="bitcoinsv:{$bitcoinsvAddress}?amount={$valueInBCH}&label=Order%3A{$orderNumber}">{$bitcoinsvAddress}</a></div>
        <div style="padding: 5px"><a target="_blank" style="background-color: white;" href="bitcoinsv:{$bitcoinsvAddress}?amount={$valueInBCH}&label=Order%3A{$orderNumber}"><img src="https://chart.googleapis.com/chart?cht=qr&chs=200x200&chl=bitcoinsv%3A{$bitcoinsvAddress}%3Famount%3D{$valueInBCH}%26label%3DOrder%3A{$orderNumber}"></a></div>
